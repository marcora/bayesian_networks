...
Let's not see a few examples for finding the independencies in a newtork using `bnlearn` and/or `dagitty`:

```{r}
# all the local independencies in the network
impliedConditionalIndependencies(g, type = "basis.set")
```

```{r}
paths(g, "D", setdiff(names(g), "D"))
```
